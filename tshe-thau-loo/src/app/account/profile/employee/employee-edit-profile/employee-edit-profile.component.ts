...
export class EmployeeEditProfileComponent implements OnInit {
  ...
  async getData(): Promise<void> {
    this.employeeService.getInfo().subscribe(
      (res: IEmployeeInfo) => { this.getSuccess(res); },
      (err: HttpErrorResponse) => { this.getFail(err); }
    );
  }

  getSuccess(res: IEmployeeInfo): void {
    this.info = res;
    this.buildForm(res);
  ...
  buildForm(data: IEmployeeInfo): void {
  ...
  async onSubmit(data: IEmployeeEditInfo): Promise<void> {
    await this.loadingService.start('修改中...');
    this.employeeService.editInfo(data).subscribe(
      (res: IEmployeeInfo) => { this.editSuccess(res); },
      (err: HttpErrorResponse) => { this.editFail(err); }
    );
  }

  async editSuccess(res: IEmployeeInfo): Promise<void> {
  ...
  async editFail(err: HttpErrorResponse): Promise<void> {
    ...
    switch (err.status) {
      ...
      case 403:
      {
        ...
        await this.router.navigate(['/account/profile/employee']);
