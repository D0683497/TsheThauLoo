...
@Component({
  selector: 'app-employee-manage-edit',
  templateUrl: './employee-manage-edit.component.html',
  styleUrls: ['./employee-manage-edit.component.scss'],
})
export class EmployeeManageEditComponent implements OnInit {
  ...
  async onSubmit(data: IEmployeeManage): Promise<void> {
    await this.loadingService.start('修改中...');
    this.employeeService.editEmployee(this.userId, data).subscribe(
      (res: IEmployee) => { this.editSuccess(res); },
      (err: HttpErrorResponse) => { this.editFail(err); }
    );
  }

  async editSuccess(res: IEmployee): Promise<void> {
  ...
  toggleIdentityConfirmed(value: CustomEvent): void {
