...
namespace TsheThauLoo.Controllers.Activity
{
    ...
    public class GeneralCampaignController : ControllerBase
    {
        ...
        public async Task<IActionResult> SignInGeneral([FromRoute] string campaignId, [FromRoute] string generalId, [FromBody] string userId)
        {
            var act = await _dbContext.GeneralCampaigns
                .AsNoTracking()
                .SingleOrDefaultAsync(x => x.CampaignId == campaignId && x.GeneralCampaignId == generalId);
            ...
            var attendee = await _dbContext.GeneralCampaignAttendees
                .SingleOrDefaultAsync(x => x.GeneralCampaignId == generalId && x.ApplicationUserId == userId);
            if (attendee == null)
            {
        ...
        public async Task<IActionResult> InviteGeneral([FromRoute] string campaignId, [FromRoute] string generalId, [FromBody] string companyId)
        {
            var act = await _dbContext.GeneralCampaigns
                .AsNoTracking()
                .SingleOrDefaultAsync(x => x.CampaignId == campaignId && x.GeneralCampaignId == generalId);
