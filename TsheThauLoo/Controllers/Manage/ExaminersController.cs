...
namespace TsheThauLoo.Controllers.Manage
{
    ...
    public class ExaminersController : ControllerBase
    {
        ...
        public ExaminersController(
            ILogger<ExaminersController> logger, 
        ...
        [AuthAuthorize(Roles = "Administrator")]
        [HttpGet(Name = nameof(GetExaminers))]
        public async Task<ActionResult<IEnumerable<ExaminerDto>>> GetExaminers([FromQuery] PaginationResourceParameters parameters)
        {
            var query = _dbContext.Examiners
                .AsNoTracking()
            ...
            var dtos = _mapper.Map<IEnumerable<ExaminerDto>>(entities);
        ...
        [AuthAuthorize(Roles = "Administrator")]
        [HttpGet("{userId}", Name = nameof(GetExaminer))]
        public async Task<ActionResult<ExaminerDto>> GetExaminer([FromRoute] string userId)
        {
            ...
            var dto = _mapper.Map<ExaminerDto>(entity);
            return Ok(dto);
        }
        
        [AuthAuthorize(Roles = "Administrator")]
        [HttpPost("{userId}", Name = nameof(EditExaminer))]
        public async Task<IActionResult> EditExaminer([FromRoute] string userId, [FromBody] ExaminerManageDto dto)
        {
            ExaminerManageDtoValidator validator = new ExaminerManageDtoValidator();
            ...
            if (result.IsValid)
            {
                ...
                var returnDto = _mapper.Map<ExaminerDto>(updateEntity);
                return CreatedAtAction(nameof(GetExaminer), routeValues, returnDto);
            }
            return BadRequest(result.Errors);
        }
    }
